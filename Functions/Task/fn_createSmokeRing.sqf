/*
    File: fn_createSmokeRing.sqf
    Author: Antigravity (Google DeepMind)
    Description: Génère un effet d'onde de choc visuelle sous forme d'un anneau de fumée en expansion locale.
*/
...
_source setParticleParams [
    ...
    [1.5, 6, 10], // Tailles successives des particules
    [
...
// Animation de l'onde qui s'élargit
while { _radius < _maxRadius && (time - _startTime) < _duration } do {
    ...
    // Ajustement de la vitesse de propulsion radiale des particules au cours du temps
    _source setParticleCircle [_radius, [2 * (1 - _progress), 2 * (1 - _progress), 0.1]];
    sleep 0.03;
};

// Nettoyage de la source de particules
